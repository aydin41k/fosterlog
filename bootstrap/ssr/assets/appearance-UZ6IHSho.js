...
const applyTheme = (appearance2) => {
  const isDark = appearance2 === "dark" || appearance2 === "system" && prefersDark();
...
function useAppearance() {
  const [appearance2, setAppearance] = useState("system");
  ...
  return { appearance: appearance2, updateAppearance };
}
function AppearanceToggleTab({ className = "", ...props }) {
  const { appearance: appearance2, updateAppearance } = useAppearance();
  const tabs = [
  ...
  return /* @__PURE__ */ jsx("div", { className: cn("inline-flex gap-1 rounded-lg bg-neutral-100 p-1 dark:bg-neutral-800", className), ...props, children: tabs.map(({ value, icon: Icon, label }) => /* @__PURE__ */ jsxs(
    "button",
    {
      ...
      className: cn(
        ...
        appearance2 === value ? "bg-white shadow-xs dark:bg-neutral-700 dark:text-neutral-100" : "text-neutral-500 hover:bg-neutral-200/60 hover:text-black dark:text-neutral-400 dark:hover:bg-neutral-700/60"
      ),
      children: [
